...
script_name="SetUpHCPPipeline_MSMRemoveGroupDrift.sh"
...
if [ "$COMPUTE" = "CHPC" ] ; then
	...
	if [ "${CLUSTER}" = "1.0" ] ; then
		...
		export CARET7DIR=/home/HCPpipeline/pipeline_tools/workbench-v1.2.0-pre1/bin_rh_linux64
		echo "${script_name}: Set up to use Workbench at ${CARET7DIR}"
		
		echo ""
		echo "${script_name}: Setting up HCP Pipelines"
		export HCPPIPEDIR=/home/HCPpipeline/pipeline_tools/Pipelines_MSMRemoveGroupDrift
		...
		echo ""
		echo "${script_name}: Setting up Workbench (a.k.a. CARET7)"
		export CARET7DIR=${HOME}/pipeline_tools/workbench-v1.2.0-pre1/bin_rh_linux64
		echo "${script_name}: Set up to use Workbench at ${CARET7DIR}"
		
		echo ""
		echo "${script_name}: Setting up HCP Pipelines"
		export HCPPIPEDIR=${HOME}/pipeline_tools/Pipelines_MSMRemoveGroupDrift
