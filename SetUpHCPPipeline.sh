...
if [ "$COMPUTE" = "" ] ; then
   echo "SetUpHCPPipeline.sh: COMPUTE value unset.  Setting it to default to NRG"
   export COMPUTE="NRG"
fi
...
if [ "$COMPUTE" = "NRG" ] ; then
    ...
    echo "${SCRIPT_NAME}: Setup to use Workbench (a.k.a. Caret7) at ${CARET7DIR}"
    ...
    # PIPELINE SPACE...
    export HCPPIPEDIR=/home/HCPpipeline/pipeline_tools/Pipelines
    echo "${SCRIPT_NAME}: Setup to use HCP Pipeline Scripts at ${HCPPIPEDIR}"
    ...
    export HCPPIPEDIR_Bin=${HCPPIPETOOLS}/binaries
