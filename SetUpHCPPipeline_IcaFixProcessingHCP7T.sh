...
echo "SetUpHCPPipeline_IcaFixProcessingHCP7T.sh: This script must be SOURCED to correctly setup the environment prior to running"
echo "SetUpHCPPipeline_IcaFixProcessingHCP7T.sh: any of the other HCP scripts contained here."

if [ "${COMPUTE}" = "" ] ; then
    echo "SetUpHCPPipeline_IcaFixProcessingHCP7T.sh: COMPUTE value unset.  Setting to the default of CHPC"
    export COMPUTE="CHPC"
fi

if [ "${COMPUTE}" = "CHPC" ] ; then
    echo "SetUpHCPPipeline_IcaFixProcessingHCP7T.sh: Setting up for processing on ${COMPUTE}"
	...
	if [ "${CLUSTER}" = "2.0" ] ; then
		...
		echo "SetUpHCPPipeline_IcaFixProcessingHCP7T.sh: Setting up FreeSurfer"
		...
		echo "SetUpHCPPipeline_IcaFixProcessingHCP7T.sh: Set up to use HCP Pipeline Scripts at ${HCPPIPEDIR}"

		# MSM
		export MSMBin=${NRG_PACKAGES}/tools.release/MSMSulc-v1.3-2014.02.10
		echo "SetUpHCPPipeline_IcaFixProcessingHCP7T.sh: Set up to use MSM binary at ${MSMBin}"

		# ICA+FIX
		export FSL_FIXDIR=${HOME}/pipeline_tools/fix1.06
		echo "SetUpHCPPipeline_IcaFixProcessingHCP7T.sh: Set up to use ICA FIX at ${FSL_FIXDIR}"

    else # unhandled value for ${CLUSTER}
		echo "SetUpHCPPipeline_IcaFixProcessingHCP7T.sh: Processing set up for cluster ${CLUSTER} is currently not supported."
		echo "SetUpHCPPipeline_IcaFixProcessingHCP7T.sh: EXITING WITH NON-ZERO EXIT STATUS (UNSUCCESSFUL EXECUTION)"
		exit 1
		
    fi
	
else # unhandled value for ${COMPUTE}
    echo "SetUpHCPPipeline_IcaFixProcessingHCP7T.sh: Processing set up for COMPUTE environment ${COMPUTE} is currently not supported."
    echo "SetUpHCPPipeline_IcaFixProcessingHCP7T.sh: EXITING WITH NON-ZERO EXIT STATUS (UNSUCCESSFUL EXECUTION)"
